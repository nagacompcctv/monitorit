...
export default function App() {
  ...
  const isAdmin = user?.role === 'head_of_it' || user?.role === 'administrator' || user?.role === 'supervisor' || user?.role === 'manager' || user?.role === 'it_admin';

  useEffect(() => {
    if (user && !isStaff && activeTab === 'timeline' && !localStorage.getItem('tab_initialized')) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        if (userDoc.exists()) {
          ...
          const adminRoles = ['head_of_it', 'administrator', 'supervisor', 'manager'];
          if (userData.email === 'wachid@duanaga.com' && !adminRoles.includes(userData.role)) {
            userData.role = 'head_of_it';
            await setDoc(doc(db, 'users', firebaseUser.uid), { role: 'head_of_it' }, { merge: true });
          }
  ...
  const handleManualLogin = async (e: React.FormEvent) => {
    ...
    try {
      const q = query(collection(db, 'users'), where('email', '==', loginEmail), where('password', '==', loginPassword));
      ...
      if (!snap.empty) {
        ...
        if (userData.email === 'wachid@duanaga.com' && !adminRoles.includes(userData.role)) {
          userData.role = 'head_of_it';
          await setDoc(doc(db, 'users', userData.uid), { role: 'head_of_it' }, { merge: true });
        }
        setUser(userData);
        ...
        >
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Pusat <span className="text-blue-600">Komando</span></h1>
              <p className="text-xs text-gray-400 font-medium uppercase">Operational Management Suite</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleManualLogin} className="space-y-4">
              ...
              {loginError && (
                <p className="text-[10px] text-red-500 font-bold uppercase">{loginError}</p>
              )}
            ...
            <div className="relative flex items-center justify-center">
              ...
              <span className="relative px-4 bg-white text-[10px] text-gray-300 font-bold uppercase">Atau</span>
            </div>
            ...
            <div className="pt-8 border-t border-gray-50">
               <p className="text-[10px] text-gray-400 font-medium uppercase">Akses khusus personel terdaftar</p>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Layout>
                ...
                {activeTab === 'assets-surveilans' && !isStaff && <Surveilans />}
                {activeTab === 'assets-flow-serah_terima' && !isStaff && <AssetFlowList mode="serah_terima" />}
                {activeTab === 'assets-flow-maintenance' && !isStaff && <AssetFlowList mode="maintenance" />}
                {activeTab === 'assets-flow-mutasi' && !isStaff && <AssetFlowList mode="mutasi" />}
                {activeTab === 'assets-flow-stock_opname' && !isStaff && <AssetFlowList mode="stock_opname" />}
                {activeTab === 'assets-flow-disposal' && !isStaff && <AssetFlowList mode="disposal" />}
                {activeTab === 'performance' && !isStaff && <Performance />}
                {activeTab === 'users' && isAdmin && <UserManagement />}
                {activeTab.startsWith('settings') && isAdmin && <SystemSettings activeTabFromProps={activeTab} />}
              </Layout>
            </motion.div>
          </AnimatePresence>
        </main>
